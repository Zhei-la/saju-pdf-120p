...
function revealPdfPreview() {
  const ids = ['step1','step2','pdfRender','previewWrap','previewArea','pdfPreview'];
...
function buildSajuTable(s) {
  ...
  const isTimeUnknown = !!currentUserInfo?.timeUnknown;
  ...
  // 둘째 줄: 천간 (한글 + 한자)
  for (let i = 0; i < 4; i++) {
    const isUnknownHour = isTimeUnknown && i === 3;
    const kor = isUnknownHour ? 'X' : (parts[i] || '·').charAt(0);
    const han = isUnknownHour ? 'X' : (partsHan[i] || '·').charAt(0);
  ...
  for (let i = 0; i < 4; i++) {
    const isUnknownHour = isTimeUnknown && i === 3;
    const kor = isUnknownHour ? 'X' : ((parts[i] || '··').charAt(1) || '·');
    const han = isUnknownHour ? 'X' : ((partsHan[i] || '··').charAt(1) || '·');
...
function pickYearlyKeywordTitle(text, fallback) {
  const t = String(text || '').replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
  const rules = [
    ['금전', '금전 점검'], ['재물', '재물 관리'], ['상관', '표현 조절'],
    ['직업', '직업 변화'], ['사업', '사업 판단'], ['책임', '책임 증가'],
    ['관계', '관계 재편'], ['인연', '인연 변화'], ['연애', '감정 흐름'],
    ['감정', '감정 조율'], ['건강', '건강 관리'], ['피로', '체력 회복'],
    ['대운', '운의 전환'], ['세운', '올해 전환점'], ['충', '관계 변동'],
    ['합', '흐름 결합'], ['기회', '기회와 도전'], ['도전', '도전 과제'],
    ['정리', '정리와 선택'], ['주의', '주의 신호'], ['변화', '변화 흐름']
  ];
  const found = rules.find(([word]) => t.includes(word));
  return found ? found[1] : fallback;
}

const usedYearlyTitles = new Set();

function yearlyKeywordTitle(idx, fallback) {
  const ch = currentChapters[idx] || {};
  let title = pickYearlyKeywordTitle(ch.body || ch.content || '', fallback);

  if (usedYearlyTitles.has(title)) {
    title = fallback;
  }

  usedYearlyTitles.add(title);
  return title;
}
...
async function downloadPDF() {
  ...
  const PARTS_BY_TYPE = {
    deep: [
      { title: 'PART 1. 사주 원국과 타고난 기질', sub: 'Core Saju Structure', range: [0, 5] },
      { title: 'PART 2. 오행과 감정 구조', sub: 'Five Elements & Emotional Flow', range: [5, 20] },
      { title: 'PART 3. 성격·관계·심리', sub: 'Personality & Relation', range: [20, 25] },
      { title: 'PART 4. 직업·재물·현실운', sub: 'Career & Money', range: [25, 35] },
      { title: 'PART 5. 연애·결혼·가족', sub: 'Love Marriage Family', range: [35, 40] },
      { title: 'PART 6. 건강·생활 패턴', sub: 'Health & Emotional Balance', range: [40, 45] },
      { title: 'PART 7. 대운·연령별 흐름', sub: 'Life Cycle Flow Flow', range: [45, 52] },
      { title: 'PART 8. 용신과 인생 방향성', sub: 'Destiny & Life Direction', range: [52, 55] }
    ],
    full: [
      { title: 'PART 1. 사주 원국과 타고난 기질', sub: 'Core Saju Structure', range: [0, 5] },
      { title: 'PART 2. 오행과 감정 구조', sub: 'Five Elements & Emotional Flow', range: [5, 20] },
    ...
    money: [
      { title: 'PART 1. 재물운 총론', sub: 'Money Overview', range: [0, 4] },
      { title: 'PART 2. 직업·커리어 분석', sub: 'Career Analysis', range: [4, 9] },
      { title: 'PART 3. 사업운 분석', sub: 'Business Fortune', range: [9, 15] },
      { title: 'PART 4. 투자·부동산·금전관리', sub: 'Investment & Asset', range: [15, 20] }
    ],
    couple: [
      { title: 'PART 1. 두 사람 기본 궁합', sub: 'Basic Match', range: [0, 3] },
      { title: 'PART 2. 연애 스타일 궁합', sub: 'Love Style Match', range: [3, 8] },
      { title: 'PART 3. 현실 궁합', sub: 'Reality Match', range: [8, 13] },
      { title: 'PART 4. 감정·집착·이별 흐름', sub: 'Emotion Breakup Reunion', range: [13, 17] },
      { title: 'PART 5. 미래 흐름', sub: 'Future Flow', range: [17, 18] }
    ]
  };
  const PARTS = currentUserInfo.reportType === 'yearly'
    ...
    : (PARTS_BY_TYPE[currentUserInfo.reportType] || ALL_PARTS);
  ...
  PARTS.forEach((part, partIdx) => {
    ...
    html += `
      <div class="pdf-page part-page report-theme-${currentUserInfo.reportType || 'deep'}" style="display:flex;flex-direction:column;justify-content:center;align-items:center;text-align:center;">
        ...
        <div style="font-size:18px;color:#b8860b;letter-spacing:8px;font-family:'Noto Sans KR',sans-serif;margin-bottom:20px;">${getReportVisualTheme().label} · PART ${partIdx + 1}</div>
    ...
    if (currentUserInfo.reportType === 'yearly' && partIdx === 5) {
      ...
      monthChapters.forEach((ch, idx) => {
        ...
        desc = desc
          .replace(/^0?\d{1,2}월은\s*/g, '')
          .replace(/^0?\d{1,2}월\s*/g, '')
          .trim();
        ...
        html += `
          <div class="visual-month-card">
            ...
            <div class="desc">${escapeHtml(month + '은 ' + desc)}</div>
          </div>
        `;
      });
      ...
      html += `
          ...
          <div class="visual-note-box">
            이 페이지는 올해 월별 흐름을 한눈에 정리한 요약 카드입니다. 자세한 흐름과 해석은 이어지는 월별 운세에서 확인할 수 있습니다.
          </div>
        </div>
      `;
    }
    ...
    if (currentUserInfo.reportType === 'yearly' && partIdx === 0) {
      ...
      const toSentenceSummary = (txt, maxSentences = 2) => {
      ...
      const getDesc = (idx, fallback) => {
        const t = currentChapters[idx]?.body || currentChapters[idx]?.content || '';
        return toSentenceSummary(t, 2) || fallback;
      };
      ...
      html += `
        ...
        <div class="pdf-page yearly-visual-page">
          ...
          <table class="yearly-compare-table">
            ...
            <tr>
              <td>
                <b style="color:#b8860b;">${escapeHtml(yearlyKeywordTitle(0, '상반기 핵심'))}</b><br>
                초반에는 관계와 생활 리듬을 정리하는 흐름이 강합니다. 무리하게 확장하기보다, 애매하게 끌고 온 문제를 정리하는 쪽이 유리합니다.
              </td>
              <td>
                <b style="color:#b8860b;">${escapeHtml(yearlyKeywordTitle(4, '하반기 핵심'))}</b><br>
    ...
    if (currentUserInfo.reportType === 'yearly' && partIdx === 0) {
      ...
      const scoreSource = [
        ...
        currentUserInfo.birthTime,
        currentUserInfo.calendarType,
        currentUserInfo.dayMaster,
        currentUserInfo.yearPillar,
        currentUserInfo.monthPillar,
        currentUserInfo.dayPillar,
        currentUserInfo.hourPillar,
        cleanText(currentChapters[0]?.body || currentChapters[0]?.content || ''),
        cleanText(currentChapters[5]?.body || currentChapters[5]?.content || ''),
        cleanText(currentChapters[10]?.body || currentChapters[10]?.content || ''),
        cleanText(currentChapters[13]?.body || currentChapters[13]?.content || '')
      ].filter(Boolean).join('|');

      const hashScore = (salt, min = 56, max = 88) => {
        const str = scoreSource + '|' + salt;
        let hash = 0;
      ...
      const bodyAll = cleanText(currentChapters.map(ch => ch?.body || ch?.content || '').join(' '));
      const countWord = (words) => words.reduce((sum, word) => sum + ((bodyAll.match(new RegExp(word, 'g')) || []).length), 0);
      const clampScore = (n) => Math.max(45, Math.min(95, Math.round(n)));

      const relationBoost = countWord(['관계', '인연', '연애', '감정', '소통']);
      const moneyBoost = countWord(['재물', '금전', '돈', '수입', '지출', '투자']);
      const careerBoost = countWord(['직업', '일', '사업', '성과', '프로젝트', '기회']);
      const healthBoost = countWord(['건강', '체력', '피로', '스트레스', '휴식']);
      const cautionBoost = countWord(['주의', '갈등', '충돌', '무리', '불안']);

      const scores = [
        ['연애운', clampScore(hashScore('love') + Math.min(relationBoost, 8) - Math.min(cautionBoost, 5))],
        ['재물운', clampScore(hashScore('money') + Math.min(moneyBoost, 8) - Math.min(cautionBoost, 4))],
        ['직업운', clampScore(hashScore('career') + Math.min(careerBoost, 8) - Math.min(cautionBoost, 3))],
        ['인간관계', clampScore(hashScore('relation') + Math.min(relationBoost, 9) - Math.min(cautionBoost, 6))],
        ['건강운', clampScore(hashScore('health') + Math.min(healthBoost, 6) - Math.min(cautionBoost, 7))]
      ];
